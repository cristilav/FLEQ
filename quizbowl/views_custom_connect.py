...
def CargarLogs(request):
	...
	usuario1 = "AUSENTE"
 	usuario2 = "AUSENTE"
 	todos = Mensajes.objects.filter(torneo="Curso Introducción a la Programación (Miriada X)").order_by('id')
 	for one in todos:
 		if one.partida == 410:
 	...
 	nuevojuego = Juegos(partida = 410, torneo ="Curso Introducción a la Programación (Miriada X)", ronda = 5, player1 = usuario1, player2 = usuario2)
 	nuevojuego.save()
 	 
 	return HttpResponse("Datos almacenados correctamente")
 	 	 		
#     f = open('/pfc-jgonzalez-data/home/jgonzalez/virtualenvs/fleq/fleq/logs/367') 
#     lines = f.readlines()
#       
#     for line in lines:
#         if line != "":
...
#               
#             mensaje = Mensajes(partida=367, torneo = "cursointroduccionalaprogramacionmiriadax", ronda = 2, hora=fecha, usuario=usuario, texto=texto, tipo = tipo, acierto = acierto,  num_question = num_question, tiempo_respuesta = tiempo_respuesta)
#             mensaje.save()
#       
#     f.close()
...
def help(request):
	...
	if request.mobile:
		return render_to_response('custom/help.html', {})
