...
def NextGames(request):
	...
	#myFutureGames = Game.objects.filter(Q(log__isnull = False), Q(start_time__gte = datetime.datetime.utcnow()),
	...
	for game in myFutureGames:
		if not game.start_time_committed and (datetime.datetime.utcnow() + datetime.timedelta(minutes=120)) > game.start_time:
...
def SelectStartTime(request, gid):
	...
	while startDate <= finishDate:
		...
		if startDate >= datetime.datetime.utcnow().date():
	...
	if request.method == "GET":
		...
		firstDateSinceNow = datetime.datetime.utcnow()
