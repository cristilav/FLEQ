...
def notify_user(user, case, instance):
	...
	if case == 'new_tournament':
		...
		timezone = int(up.timezone)*60
		fecha = game.start_time + datetime.timedelta(minutes=timezone)
		...
		message += " del torneo '" + unicode(tournament) + u"' a la que estás convocado se celebrará el día " 
		message += fecha.strftime("%d/%m/%Y") + " a las " +  fecha.strftime("%H:%M%Z") + " horas.\n"
		...
		message += " del torneo '" + unicode(game.round.tournament) + u"' empezará en unos minutos (" + fecha.strftime("%H:%M%Z") + ")." 
		message += "\n\n\n"
		...
		up= UserProfile.objects.get(user=user)
		timezone = int(up.timezone)*60
		fecha = tournament.start_date + datetime.timedelta(minutes=timezone)		
		...
		if user == tournament.admin: 
			...
			message += "\t- Fecha de comienzo: " + fecha.strftime("%d/%m/%Y") + "\n" 
		...
		message += u"En caso de que ambos jugadores no alcancen un acuerdo en la hora de preferencia, "
		message += u"prevalecerá la hora establecida por defecto por FLEQ, es decir, las 21:00 hrs. UTC (Hora Universal Coordinada) del último día de la ronda "
		message += u"(si la ronda dura un sólo día, dicha hora corresponderá al día de hoy)."
		message += "\n\n"
		message += u"IMPORTANTE: los horarios que aparecen en FLEQ son adaptados a la zona horaria de cada usuario."
		message += "\n\n\n"
